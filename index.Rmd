...
2. Put some of those values in the boxplot y-axis:
...
This boxplot shows outliers on the right tail, that means there is theoretical upper limit or threshold different than the maximum value of the variable. I can get that threshold this way:

  a. Compute the interquartile range (difference between q3 and q1):

```{r, eval=TRUE}
(theIQR=IQR(eduwa$Reduced.Lunch,na.rm = T))
```

  b. Multiply the IQR by a factor (typically 1.5). That value is added to the third quartile and the result is the upper threshold (if there were a lower threshold you would substract):
...
```
  ...
  c. Knowing the upper threshold, I can compute the amount of outliers in the right tail:

```{r, eval=TRUE}
(numOutliers=sum(eduwa$Reduced.Lunch>upperT,na.rm = T))
```

3. We can _decide_ to annotate my boxplot with this value:
...
Also, change text location in the horizontal:
```{r}
...
- Your  **own good and alternative** version of a Pareto plot for the _Free.Lunch_ variable. This would be your first step:
