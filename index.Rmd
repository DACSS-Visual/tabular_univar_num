...
2. Put those values in the boxplot y-axis (y in the horizontal - as I flipped the plot):
...
Knowing the upper threshold, I can compute the _amount of outliers_ on the right tail:
...
txtThreshold=paste0('Threshold:',upperT)
...
```{r}
b1_annot_noX + theme(axis.text.x = element_text(angle = 60,
                                                         size = 7,
                                                         vjust = 0.5))
```
...
- Your  **own alternative** version of a Pareto plot for the _Free.Lunch_ variable without using the library **ggQC** . This would be your first step:
